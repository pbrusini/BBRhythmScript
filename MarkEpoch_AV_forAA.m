...
%     try
        ...
        try
            ...
            EEGR = pop_epoch( EEG, { 'TRS0' }, [-0.3  8], 'newname', 'EGI file epochs', 'epochinfo', 'yes');
            % [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, CURRENTSET,'setname', 'EGI file epochs', 'save', setNamesegNx);
            EEGR = pop_rmbase( EEGR, [-300  8000]);
            EEGR = pop_saveset( EEGR, NamSetResting, pathOut);
            
            %   [ALLEEG EEGR CURRENTSET] = eeg_store(ALLEEG, EEGR);
        end
        try
            EEGD = pop_epoch( EEG, { 'DIND' }, [-0.3  8], 'newname', 'EGI file epochs', 'epochinfo', 'yes');
            % [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, CURRENTSET,'setname', 'EGI file epochs', 'save', setNamesegNx);
            EEGD = pop_rmbase( EEGD, [-300  8000]);
            EEGD = pop_saveset( EEGD, NamSetDrum, pathOut);
            
        end
        try
            
            EEGS = pop_epoch( EEG, { 'DINV' }, [-0.3  8], 'newname', 'EGI file epochs', 'epochinfo', 'yes');
            % [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, CURRENTSET,'setname', 'EGI file epochs', 'save', setNamesegNx);
