pathIn='/media/Work/Data_RhythmProject/Data_Analysis/EEG_Analysis/2months/Event_Filtered_MarkedbyTrial_CleanByProb_RerefDetrendBase_FFTSNR/';
pathOut='/media/Work/Data_RhythmProject/Data_Analysis/EEG_Analysis/2months/Event_Filtered_MarkedbyTrial_CleanByProb_RerefDetrendBase_FFTSNR_Cluster/';
...
for i=1:length(ssListDrum)
    ...
    tmpmeanPhase(i,:,:,:)=squeeze(mean(TFdata.phase,1));
    if size(TFdata.phase,1)<25
        for i=size(TFdata.phase,1):25
            TFdata.phase(i,:,:)=zeros(64,4000);
        end
    end
    tmpPhase(i,:,:,:)=TFdata.phase;
