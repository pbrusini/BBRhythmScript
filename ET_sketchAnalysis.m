...
ETHeader=[timeStampHeader eyeArrayHeader_Left eyeArrayHeader_Right trigSignalHeader];
try
    lookET=[Nom.ET(:,1) Nom.ET(:,8) Nom.ET(:,9) Nom.ET(:,14) Nom.ET(:,22) Nom.ET(:,23) Nom.ET(:,28) Nom.ET(:,29) Nom.ET(:,30)];
catch
    lookET=[ETdata(:,1) ETdata(:,8) ETdata(:,9) ETdata(:,14) ETdata(:,22) ETdata(:,23) ETdata(:,28) ETdata(:,29) ETdata(:,30)];
...
for i=1:length(lookET)
    if lookET(i,4)==0
        LeftXY=[LeftXY; lookET(i,2) lookET(i,3)];
    end
end
...
for i=1:length(lookET)
    if lookET(i,7)==0
        RightXY=[RightXY; lookET(i,5) lookET(i,6)];
    end
end

figure(4)
plot(LeftXY(:,1), LeftXY(:,2))
hold on
plot(RightXY(:,1), RightXY(:,2), 'r')
...
while v<length(TrackObjt)
   v=v+1;
    ...
    if TrackObjt(v,2)==1 || TrackObjt(v,2)==2
        ...
        if TrackObjt(v,5) == TrackObjt(v,6)
            lookTarget=lookTarget+1;
            Trial=Trial+1;
        elseif TrackObjt(v,6) == 0
            lookScreen=lookScreen+1;
            Trial=Trial+1;
        elseif TrackObjt(v,6) == -1
            lookNothing=lookNothing+1;
            Trial=Trial+1;
        elseif TrackObjt(v,6)~= TrackObjt(v,5)
