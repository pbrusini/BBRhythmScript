...
figure(3)
subplot(3,1,1);
[l,p] = boundedline(freq(7:70), LL_Drum_twomo(7:70), 2*LL_Drum_twomostd(7:70), 'b', freq(7:70), LL_Syll_twomo(7:70), 2*LL_Syll_twomostd(7:70), 'r', freq(7:70), LL_Rest_twomo(7:70), 2*LL_Rest_twomostd(7:70), 'black','alpha');
%outlinebounds(l,p);
title('Frequency power all Condition 2mo');
subplot(3,1,2);
[l,p] = boundedline(freq(7:70), LL_Drum_sixmo(7:70), 2*LL_Drum_sixmostd(7:70), 'b', freq(7:70), LL_Syll_sixmo(7:70), 2*LL_Syll_sixmostd(7:70), 'r', freq(7:70), LL_Rest_sixmo(7:70), 2*LL_Rest_sixmostd(7:70), 'black','alpha');
%outlinebounds(l,p);
title('Frequency power all Condition 6mo');
subplot(3,1,3);
[l,p] = boundedline(freq(7:70), LL_Drum_ninemo(7:70), 2*LL_Drum_ninemostd(7:70), 'b', freq(7:70), LL_Syll_ninemo(7:70), 2*LL_Syll_ninemostd(7:70), 'r', freq(7:70), LL_Rest_ninemo(7:70), 2*LL_Rest_ninemostd(7:70), 'black','alpha');
%outlinebounds(l,p);
...
FrontClust=[6 9 10 11 12 13 14 19];
...
for b=1:length(BBdatabase)
    try
        ...
        %        DiffSyl= TFGrandSylldataTwoMo.powspctrm(BBdatabase(b).EEGTwomo.Ta,:,:);
        %        DiffDru=TFGrandDrumdataTwoMo.powspctrm((BBdatabase(b).EEGTwomo.Drum),:,:);
        BB2monthAllcond(i)=BBdatabase(b);
        
        EEGdat(i).SylFrontClust=mean(mean(DiffSyl(:,FrontClust,twoHz),2),3);
        EEGdat(i).SylParClust=mean(mean(DiffSyl(:,ParClust,twoHz),2),3);
        
        %
        EEGdat(i).DrumFrontClust=mean(mean(DiffDru(:,FrontClust,twoHz),2),3);
        EEGdat(i).DrumParClust=mean(mean(DiffDru(:,ParClust,twoHz),2),3);
        %
        Behav(i).Sex=BBdatabase(b).Sex;
        Behav(i).dob=BBdatabase(b).DOB;
        Behav(i).asqTwo=BBdatabase(b).TotalTwomo;
        Behav(i).asqSix=BBdatabase(b).SixmoASQ.Communication;
...
for i=1:length(select2mo)
    ...
    data2=[data2; Behav(select2mo(i)).asqTwo];
end
...
data2=[];
data3=[];
select2mo=[1:3 5 7:9 14 16]
%select2mo=[1:17]
for i=1:length(select2mo)
    data1=[data1; EEGdat(select2mo(i)).SylFrontClust];
    data3=[data3; EEGdat(select2mo(i)).DrumFrontClust];
    data2=[data2; BB2monthAllcond(select2mo(i)).EEGFourmo];
end
[rpc fig sstruct]=BlandAltman(data1,data2, {'Pow Syl-Rest for FrontClust', 'ASQ score'})
[rpc fig sstruct]=BlandAltman(data3,data2,  {'Pow Drum-Rest for FrontClust', 'ASQ score'})
...
select2mo=[1:12 14:17]
for i=1:length(select2mo)
    data1=[data1; EEGdat(select2mo(i)).SylParClust];
    data3=[data3; EEGdat(select2mo(i)).DrumParClust];
    data2=[data2; Behav(select2mo(i)).asqSix]
end
[rpc fig sstruct]=BlandAltman(data1,data2,  {'Pow Syl-Rest for ParClust', 'ASQ 6mo score'})
[rpc fig sstruct]=BlandAltman(data3,data2,  {'Pow Drum-Rest for ParClust', 'ASQ 6mo score'})

data1=[];
data2=[];
data3=[];
%select2mo=[1:6 9:13 15:18]
select2mo=[1:16 18:19 21:23]
select2mo=[1:12 14:17]
for i=1:length(select2mo)
    data1=[data1; EEGdat(select2mo(i)).SylFrontClust];
    data3=[data3; EEGdat(select2mo(i)).DrumFrontClust];
...
[rpc fig sstruct]=BlandAltman(data1,data2,  {'Pow Syl-Rest for FrontClust', 'ASQ 6mo score'})
[rpc fig sstruct]=BlandAltman(data3,data2,  {'Pow Drum-Rest for FrontClust', 'ASQ 6mo score'})
